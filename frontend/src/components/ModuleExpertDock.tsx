...
/** Tope del historial persistido, en caracteres.
 *  Las respuestas del asistente son Markdown largo: 30 turnos por módulo podían
 *  ocupar cientos de KB del cupo de localStorage, el mismo que usan los
 *  diagramas BPMN. Se recortan los turnos más antiguos hasta caber. */
const MAX_HISTORY_CHARS = 120_000;

function trimHistory(history: Exchange[]): Exchange[] {
  const kept = history.filter((h) => !h.loading).slice(-30);
  while (kept.length > 1 && JSON.stringify(kept).length > MAX_HISTORY_CHARS) {
    kept.shift();
  }
  return kept;
}
...
export function ModuleExpertDock({ activeModule }: Props) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(historyKey(activeModule), JSON.stringify(trimHistory(history)));
    } catch { /* ignore */ }
