...
/** Agrupa los diagramas BPMN por el ítem del mapa al que pertenecen. */
function groupByMapItem(diagrams: BpmnDiagramSummary[]): Record<string, string[]> {
  const out: Record<string, string[]> = {};
  for (const d of diagrams) {
    if (d.map_item_id) (out[d.map_item_id] ??= []).push((d.name || "Proceso").trim() || "Proceso");
  }
  return out;
}
...
export function MapaProcesosTab({ company, onCompanyUpdated }: {
  ...
  // Los diagramas se guardan en el backend, así que hay que pedirlos.
  const [bpmnByItem, setBpmnByItem] = useState<Record<string, string[]>>({});
  useEffect(() => {
    let cancelled = false;
    listBpmnDiagrams(company.id)
      .then((ds) => { if (!cancelled) setBpmnByItem(groupByMapItem(ds)); })
      .catch(() => { if (!cancelled) setBpmnByItem({}); });
