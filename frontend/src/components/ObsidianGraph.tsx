...
function buildLocalOrgGraph(company: Company, diagrams: BpmnDiagramSummary[]): OrgGraph {
  ...
  // Diagramas BPMN (guardados en el backend): cada uno cuelga de su proceso del
  // mapa, o de la empresa si no está ubicado, con sus escenarios AS-IS/TO-BE.
  for (const proc of diagrams) {
    const hasAsis = proc.has_asis;
    const hasTobe = proc.has_tobe;
    const dgName = (proc.name || "").trim();
    if (!dgName && !hasAsis && !hasTobe) continue;
    const located = proc.map_item_id ? items.find((i) => i.id === proc.map_item_id) : undefined;
...
export function ObsidianGraph({ company }: { company?: Company }) {
  ...
  useEffect(() => {
    ...
    if (company) {
      let cancelled = false;
      // Los diagramas viven en el backend; si no responde, se dibuja el resto.
      listBpmnDiagrams(company.id)
        .catch(() => [] as BpmnDiagramSummary[])
        .then((diagrams) => {
          if (cancelled) return;
          try { setGraph(buildLocalOrgGraph(company, diagrams)); }
