...
/**
 * Valor esperado (min) de la distribución, con los mismos valores por defecto
 * que `sample`. En la uniforme y la triangular la media no se usa: dependen de
 * mínimo, moda y máximo.
 */
export function expectedValue(d: Distribution): number {
  switch (d.kind) {
    case "uniform": {
      const lo = d.min ?? Math.max(d.mean * 0.5, 0);
      const hi = d.max ?? d.mean * 1.5;
